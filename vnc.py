...
def enterNetworkName(networkName, recentNetworkNames = None):
    ...
    networkNameUserInput = str(input("     enter/change networkName (only! ENTER to use current networkName):\n     "))
...
def main():
    ...
    while menuInput != "q":
        ...
        if menuInput == "a": 
            ...
            if vascularNetwork.vessels != {}:
                ...
                while True:
                    try:
                        mother = int(mother)
                        if mother in list(vascularNetwork.vessels.keys()) and vascularNetwork.vessels[mother].rightDaughter == None:
                            break
                        else:
                            existing = True
                    except ValueError:
            ...
            complianceTypes = list(nxml.vesselComplianceElements.keys())
        ...
        if menuInput == "d":
            ...
            if list(vascularNetwork.vessels.keys()) != []:
...
#                     print "     [5] - write boundary conditions to CSV"
                    ...
                    if subMenuInput == '1':
                        ...
                        if len(list(vascularNetwork.boundaryConditions.keys())) == 1:
                        ...
                        vesselId = -1
                        while vesselId not in list(vascularNetwork.boundaryConditions.keys()):
...
#                         mCSV.writeBCToCSV(networkName, boundaryConditions, boundaryConditionPolyChaos)    
            ...
            while  subMenuInput not in ["1","2","b"]:
                ...
                for key,value in vascularNetwork.globalFluid.items():
                ...
                if subMenuInput == '1':
                    ...
                    for key in list(vascularNetwork.globalFluid.keys()):
                        inputType = "1"
                        typeFalse = False
