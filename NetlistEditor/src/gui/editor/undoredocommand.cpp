...
bool MoveItems::mergeWith(const QUndoCommand* command)
{
  ...
  const QSet<QGraphicsItem*> commandItems(
      static_cast<const MoveItems*>(command)->items_.begin(),
      static_cast<const MoveItems*>(command)->items_.end());
  const QSet<QGraphicsItem*> currentItems(items_.begin(), items_.end());
  if(commandItems == currentItems) {
