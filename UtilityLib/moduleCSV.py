...
def writeVesselDataToCSV(networkName, vessels, delimiter=';'):
    ...
    for vesselElement in nxml.vesselElements:
        if vesselElement == 'compliance':
            for specificCompElements in list(nxml.vesselElementReference[vesselElement].values()):
...
def readVesselDataFromCSV(networkName, delimiter=';'):
    ...
    with open(vesselCSVFile,'r', newline='') as csvfile:
        ...
        variablesToDiscard = []
        for Id,data in vesselData.items():
    #         polyChaos = {}
            for variable,variableValueStr in data.items():
...
def writeRandomInputstoCSV(networkName, randomInputManager, delimiter = ';'):
    ...
    for listValue in nxml.randomInputsReference.values():
...
#                 if variable != 'name': polyChaosTags[variable] = len(interval)     
    ...
    tagsBCType2 = [] 
    for boundaryCondition,elementTags in nxml.boundaryConditionElements.items():
...
#                                 tagsBCType2.append(''.join([elementTag,'-pC',str(int(count)+1)]))
    ...
    for Id,boundaryConditions in boundaryConditionDict.items():
...
def readBCFromCSV(networkName, delimiter=';'):
    ...
    for bcData in boundaryData:
        ...
        polyChaos = {}
        for variable,variableValueStr in bcData.items():
            ...
            if '-pC' in variable and variableValueStr != '':
                ...
                if variable in list(polyChaos.keys()):
        ...
        if polyChaos != {}:
            for variable,variableValueStr in polyChaos.items():
                try:
            ...
            if Id not in list(boundaryConditionPolyChaos.keys()): 
        ...
        if Id not in list(BCconditionData.keys()): BCconditionData[Id] = [boundaryInstance]
