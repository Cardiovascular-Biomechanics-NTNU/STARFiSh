...
class Visualisation2DPlotWindowAdjustValues(gtk.Window):
    ...
    def on_clickedUpdate(self, widget):
        ...
        '''
        for key, entries in self.entriesCombo.items():
...
class Visualisation2DPlotWindow(Visualisation2DPlotWindowGui):
    ...
    def update(self, visualisationData):
        ...
        '''
        for key,value in visualisationData.items():
            try:
    ...
    def createGraph(self):
        ...
        for caseId in range(len(self.selectedVesselIds)):
            ...
            self.lines[caseId]  = {key: {} for key in list(self.axis.keys())}
            self.points[caseId] = {key: {} for key in list(self.axis.keys())}
            # fill in lines according to defined axis, linesytles and color 
            for axis in list(self.axis.keys()):
    ...
    def clearPlotWindow(self, lines = True, points = True, labels = False):
        ...
        '''
        for caseId,axisDict in self.lines.items():
            for axis,lineDict in axisDict.items():
                if lines:
                    for line in lineDict.values():
                        line.set_data([-1], [0])
                ...
                if points:
                    try:
                        for point in self.points[caseId][axis].values():
                            point.set_data([-1], [0])
                    except: pass
                if labels:
                    for linestyle in lineDict.keys():
                        currentName = ' '.join([str(caseId),axis,linestyle])
                        self.labels[currentName] = '' 
    ...
    def updateLineProperties(self):
        ...
        for caseId,axisDict in self.lines.items():
            for axis,lineDict in axisDict.items():
                for linestyle,line in lineDict.items():
                    currentName = ' '.join([str(caseId),axis,linestyle])
...
class Visualisation2DMain(Visualisation2DMainGUI):
    ...
    def on_clickedPlots(self, widget):
        ...
        # # open plot window        
        if selectedNetworks != []:
            for networkID in loadVesselIDdict.keys():
