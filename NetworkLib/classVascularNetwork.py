...
class VascularNetwork(cSBO.StarfishBaseObject):
    ...
    def update(self, vascularNetworkData):
        ...
        """
        for key, value in vascularNetworkData.items():
            if hasattr(self,key):
    ...
    def updateNetwork(self, updateDict):
        ...
        if 'vesselData' in updateDict:
            for vesselId, vesselData in (updateDict['vesselData']).items():
                try:
    ...
    def initialize(self, initializeForSimulation=False):
        ...
        for fluidItem, value in self.globalFluid.items():
            if value == None:
                if fluidItem == 'dlt':
                    try:
        ...
        if self.boundaryConditions != {}:
            ...
            if len(self.vessels) == 1:
                ...
                for vesselId, bcs in self.boundaryConditions.items():
        ...
        atBCButNotDefined = set([self.root] + self.boundaryVessels).difference(list(self.boundaryConditions.keys()))
        ...
        if len(self.vessels) == 1:
            bcPositions = []
            for Id, bcs in self.boundaryConditions.items():
                for bc in bcs:
    ...
    def initializeNetworkForSimulation(self):
        ...
        for bcList in self.boundaryConditions.values():
            for bc in bcList:
        ...
        for vesselId, vessel in self.vessels.items():
        ...
        for vesselId, boundaryConditions in self.boundaryConditions.items():
        ...
        for stimulus in self.externalStimuli.values():
    ...
    def initializeHeadUpTilt(self, headUpTilt):
        ...
        for vesselId, angleDict in motionDict.items():
    ...
    def linkSolutionData(self):
        ...
        vesselId = None
        for groupName, group in self.solutionDataFile.items():
    ...
    def loadSolutionDataRange(self, vesselIds = None, tspan=None, mindt=None,
        ...
        if self._checkAccessInputs(t1, t2, mindt):
            ...
            if vesselIds == None: vesselIds = list(self.vessels.keys())
    ...
    def evaluateNetworkResistanceAndCompliance(self):
        ...
        for vesselId, vessel_i in self.vessels.items():
        ...
        for bcs in self.boundaryConditions.values():
            for bc in bcs:
